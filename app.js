...
async function loadBinaryTree() {
    try {
        treeContainer.innerHTML = '<div class="loading">در حال بارگذاری ساختار درختی شبکه...</div>';
        ...
        treeContainer.innerHTML = '<p>خطا در بارگذاری ساختار درختی</p>';
    }
}
...
function createTreeNode(node) {
    ...
    nodeContent.className = `node-box ${node.isCurrentUser ? 'current-user' : ''} ${node.error ? 'error-node' : ''}`;
